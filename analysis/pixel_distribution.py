...
def compare_pixel_distributions(
    ...
    Uses KS test for statistical comparison. For the T channel, bimodality is
    detected via peak finding in log10 space.
    ...
    for ci, ch in enumerate(field_names):
        ...
        # Bimodality detection for T channel
        if ch == "T":
            peaks, _ = find_peaks(pdf_true, prominence=pdf_true.max() * 0.1)
            entry["bimodal"] = bool(len(peaks) >= 2)
