...
from analysis.eval_integration import (
    cv_advanced_metrics, LHAdvancedAccumulator, lh_overall_pass,
...
def _eval_cv(maps_true, sim_ids, params_phys,
    ...
    for ch in CH_NAMES:
        ...
        auto_pk_res[ch] = check_auto_pk(k_arr, rel, ch)

    dcv_arr = {ch: d_cv(pks_true_sim[ch], pks_gen[ch]).tolist() for ch in CH_NAMES}
    # variance_ratio: 개별 맵 분포끼리 비교 (pks_true 405개 vs pks_gen 50개)
    ...
    overall = (all(auto_pk_res[ch]["passed"]  for ch in CH_NAMES) and
               all(cross_res[p]["passed"]     for p in PAIR_NAMES) and
               all(cohere_res[p]["passed"]    for p in PAIR_NAMES) and
               all(pdf_res[ch]["passed"]      for ch in CH_NAMES))
    ...
    return {
        ...
        "auto_pk": auto_pk_res, "cross_pk": cross_res, "coherence": cohere_res,
        "pdf": pdf_res, "field_mean": fm_stats,
        "d_cv": dcv_arr, "variance_ratio": vr_arr,
        # [new]
        "conditional_z":     adv_cv["conditional_z"],
        "r_sigma_ci":        adv_cv["r_sigma_ci"],
        "coherence_delta_z": adv_cv["coherence_delta_z"],
        "scattering":        adv_cv["scattering"],
    }
...
def _eval_lh(maps_true, params_phys, sim_ids,
    ...
    for sim in tqdm(unique_sims, desc="eval/lh sims", unit="sim"):
        ...
        cpks_t = _compute_all_cross_pk(m_true)
        cpks_g = _compute_all_cross_pk(m_gen)
        r_t    = _compute_all_coherence(m_true)
        r_g    = _compute_all_coherence(m_gen)
        r_xi, xi_t = _compute_all_xi(m_true)
        _,    xi_g = _compute_all_xi(m_gen)
        ...
        if lh_advanced_acc is None:
            lh_advanced_acc = LHAdvancedAccumulator(
                n_eff_json=N_EFF_JSON, k_arr=k_arr, n_eff_cap=None,
            )
...
def _eval_ex(maps_true, params_phys, sim_ids,
    ...
    cv_summary_path = out_dir.parent / out_dir.name.replace("eval_ex", "eval_cv") / \
...
def _print_summary(summary: dict, split: str):
    ...
    if split == "cv":
        ...
        print("  Auto P(k):")
        for ch in CH_NAMES:
            res = summary["auto_pk"][ch]
            for label in ["low_k", "mid_k", "high_k"]:
                r = res[label]
                p = "✓" if r["passed"] else "✗"
                ref = "[ref]" if r.get("reference_only") else ""
                print(f"    {ch}/{label}: mean={r['mean_err']:.3f}(≤{r['thr_mean']:.2f}) "
                      f"rms={r['rms_err']:.3f}(≤{r['thr_rms']:.2f})  {p} {ref}")
        print("\n  Cross P(k):")
        for pair in PAIR_NAMES:
            r = summary["cross_pk"][pair]
            print(f"    {pair}: {r['mean_err']:.3f}(≤{r['thr']:.2f})  "
                  f"{'✓' if r['passed'] else '✗'}")
        print("\n  Coherence:")
        for pair in PAIR_NAMES:
            r = summary["coherence"][pair]
        ...
        if "conditional_z" in summary:
            print("\n  Conditional z(k) (mean |z| per band):")
            for ch in CH_NAMES:
                ...
                slices = {
                    "low_k":  z[:thirds],
                    "mid_k":  z[thirds:2*thirds],
                    "high_k": z[2*thirds:],
                }
                parts = "  ".join(f"{band}={np.abs(zv).mean():.2f}"
                                  for band, zv in slices.items() if len(zv) > 0)
                print(f"    {ch}: {parts}")
